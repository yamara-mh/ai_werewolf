...
function buildPrecisionSpeechUserPrompt({ player, day, alivePlayersText, nextSpeakerCandidatesText, previousDaysSynopsis, todayPosts, wolfPosts, seerResults, hunterResult, mediumResults, currentVotes }) {
  ...
  lines.push(JSON.stringify({
    posts: [{ name: '（必須）', coRole: 'カミングアウトする役職ID（省略可）', thinking: '思考内容（省略可）', talk: '発言内容（必須。10～30文字）', status: '表情（必須）', villager: { name: '白だしするプレイヤー名（省略可）' }, werewolf: { name: '黒だしするプレイヤー名（省略可）' }, vote: '投票先プレイヤー名（省略可）' },
      { name: '（必須）', talk: '連投可能（必須。10～30文字）', status: '表情（必須）' }
    ],
